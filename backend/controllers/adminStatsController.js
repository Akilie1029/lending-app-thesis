...
async function getDashboardStats(req, res) {
  try {
    console.log(
      LOG_PREFIX,
      "getDashboardStats called by admin:",
      req.user?.id || "unknown"
    );
    ...
    const [
      ...
      db.query(`
        ...
        WHERE LOWER(status) = 'active'
          AND latest_due_date IS NOT NULL
          AND CURRENT_DATE > latest_due_date::date
      `),
    ...
    const overdueCount = num(overdueCountRes.rows[0]?.count);
      ...
      SELECT
        COALESCE(
          SUM(
            ...
            THEN COALESCE(disbursed_amount, total_payable, 0)
            END
          ),
          0
        ) AS paid_amount,
    ...
    const performancePercent =
      totalPayable === 0
        ? 0
        : Math.round((totalRepaid / totalPayable) * 100);
    ...
    const riskPercent =
      activePrincipal === 0
        ? 0
        : Math.round((riskExposure / activePrincipal) * 100);
    ...
    return res.json({
      ...
      loanStatusDistribution: {
        paidAmount,
        unpaidAmount,
        overdueAmount: 0, // legacy placeholder
      },
